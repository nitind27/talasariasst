...
// Copy public folder image to tem/pages folder
export async function copyPublicImageToTemPages(publicPath) {
  try {
    ...
    const ext = path.extname(cleanPath) || ".jpeg";
    const name = `${Date.now()}-${randomUUID()}${ext}`;
    const destDir = path.join(process.cwd(), "tem", "pages");
    ...
    console.error(`Error copying image ${publicPath}:`, error);
    return null;
  }
}